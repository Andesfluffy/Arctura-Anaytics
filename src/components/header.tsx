...
export function Header() {
  ...
  return (
    <header className={`sticky top-0 z-50 border-b border-white/10 ${scrolled ? 'bg-oxford_blue/95' : 'bg-oxford_blue/90'} backdrop-blur supports-[backdrop-filter]:bg-oxford_blue/85`}>
      {/* Utility bar */}
      <div className="hidden border-b border-white/10 bg-black/40 text-xs text-slate-300 md:block">
        <Container className="flex h-8 items-center justify-between">
          <div className="flex items-center gap-6">
            <span className="flex items-center gap-2"><Globe className="h-3.5 w-3.5" /> Remote‑first</span>
            <span className="flex items-center gap-2"><Clock className="h-3.5 w-3.5" /> Mon–Fri 9–5</span>
            <a className="flex items-center gap-2 hover:text-white" href="mailto:hello@arctura-analytics.com">
      ...
      <Container className="flex h-20 items-center justify-between">
        ...
        <button className="md:hidden rounded-md border border-white/10 bg-white/5 p-2" aria-label={open ? 'Close menu' : 'Open menu'} onClick={() => setOpen((v) => !v)}>
